...
function test_ci_configuration()
    @testset "CI configuration" begin
        ...
        read_config(path) = replace(read(path, String), "\r\n" => "\n")
        ci_config         = read_config(joinpath(workflows, "ci.yml"))
        docs_config       = read_config(joinpath(workflows, "docs.yml"))
        benchmark_config  = read_config(joinpath(workflows, "benchmark.yml"))
        dependabot_config = read_config(joinpath(repo_root, ".github", "dependabot.yml"))
