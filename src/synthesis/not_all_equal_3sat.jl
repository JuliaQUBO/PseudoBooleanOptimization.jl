...
function not_all_equal_3sat(rng, ::Type{F}, n::Integer, m::Integer) where {V,T,F<:AbstractFunction{V,T}}
    ...
    # Convert to boolean
    # s = 2x - 1
    return sum([J[i,j] * F(i => T(2), T(-1)) * F(j => T(2), T(-1)) for (i,j) in keys(J)])
end

function not_all_equal_3sat(::Type{F}, n::Integer, m::Integer) where {V,T,F<:AbstractFunction{V,T}}
